...
Eigen::Matrix3d Quaternion::crossMatrix(const Eigen::Vector3d &v) {
    Eigen::Matrix3d m;
    m << 0, -1*v(2), v(1),
         v(2), 0, -1*v(0),
         -1*v(1), v(0), 0;
...
Eigen::Matrix4d Quaternion::bigOmegaMatrix(const Eigen::Vector3d &v) {
    Eigen::Matrix4d m;
    m(3, 3) = 0.0;
    m.block<3, 3>(0, 0) = -1*Quaternion::crossMatrix(v);
    m.block<3, 1>(0, 3) = v;
